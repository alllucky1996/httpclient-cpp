...
std::wstring Utf8ToUtf16(const std::string& dataUtf8)
{
    std::wstring utf16;
    utf16.reserve(dataUtf8.size());
    for (size_t i = 0; i < dataUtf8.size(); ++i) {
        unsigned char ch0 = dataUtf8[i];
        if ((ch0 & 0x80) == 0x00) {
            utf16 += ((ch0 & 0x7f));
        }
        else {
            if ((ch0 & 0xe0) == 0xc0) {
                unsigned char ch1 = dataUtf8[++i];
                utf16 += ((ch0 & 0x3f) << 6) | ((ch1 & 0x3f));
            }
            else {
                unsigned char ch1 = dataUtf8[++i];
                unsigned char ch2 = dataUtf8[++i];
